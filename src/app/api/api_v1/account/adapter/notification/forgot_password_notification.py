...
class ForgotPasswordNotification(Notification[User]):
    ...
    async def to_mail(self):
        ...
        await EmailService([self.notifiable.email]).sendVerificationCode(template_data={ "name": self.notifiable.full_name(), "url": url })
        
    async def send(self):
        from loguru import logger
        # logger.debug(self.via())
        if Notification.Channels.EMAIL in self.via():
            return await self.to_mail()
        else:
            raise Exception